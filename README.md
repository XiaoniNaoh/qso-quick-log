...
呼号一输，逐项填完，点一下「保存并继续」，一条 QSO 就这么记好了——是不是很顺？
...
- 逐项点选填写：呼号起头，老呼号自动带出上次信息，填完点「保存并继续」立刻开下一条；
- 桌面端随时按 `⌘/Ctrl`+`Enter` 也能保存当前记录。
...
- 呼号起头，逐项填写，老呼号自动带出上次信息，点「保存并继续」开新一条；
- 老呼号自动带出上次的信息，不用重复输入；
...
- 保存常用中继台，填下行频率 + 上/下差偏移，自动算好上行频率；
- 首页记通联时填的新中继台也会自动入库，下次直接从列表里选，频率、差频、哑音一次带齐；
- 同频率的中继台会自动合并，避免重复。
